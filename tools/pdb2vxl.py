...
Every shipped solution is verified against the reference engine. A level whose
shipped solution fails is NEVER dropped silently: it is printed, recorded in
tools/known-bad.md, and either replaced from tools/solution-overrides.json (a
solver-found solution, re-verified here) or dropped-with-record. Duplicate boards
across packs are kept in the first pack by order and dropped from the rest.
The run only exits non-zero on STRUCTURAL PDB errors (bad header/creator, chunk
pair-count mismatch) — never for a single bad level.
...
# --------------------------------------------------------------------------
# Bad-level handling: overrides + known-bad.md
# --------------------------------------------------------------------------
...
def write_known_bad(records):
    """Write tools/known-bad.md — one entry per level whose shipped solution
    failed verification, with what happened to it. Committed to the repo so a
    dropped or replaced level is never a silent surprise."""
    today = datetime.date.today().isoformat()
    lines = [
        "# Known bad levels",
        "",
        "Auto-generated by `tools/pdb2vxl.py`. Every level whose **shipped** "
        "solution fails verification against the reference engine is listed here "
        "— either replaced with a solver-found solution (see "
        "`tools/solution-overrides.json` / `tools/solve.py`) or dropped. Nothing "
        "is ever dropped silently.",
        "",
    ]
    if not records:
        lines.append("_No bad levels: every shipped solution verifies._")
    for r in records:
        lines += [
            f"## {r['pack']} / {r['title']} (level {r['index']})",
            "",
            f"- board: `{r['board']}`",
            f"- shipped solution: `{r['original']}`",
            f"- error: {r['error']}",
...
def main():
    ...
    #   bad     -- a single level whose SHIPPED solution fails verification (or is
    #              missing). NEVER silently dropped: each is printed, recorded in
    #              known-bad.md, and either replaced from solution-overrides.json
    #              (a solver-found solution, re-verified here) or dropped-with-record.
    ...
    for pack in packs:
        kept = []
        for i, lv in enumerate(pack["levels"]):
            total_in += 1
            board, sol, title = lv.get("board"), lv.get("solution"), lv.get("title", "")

            if not board or not sol:
                bad.append(dict(pack=pack["id"], title=title, index=i, board=board or "",
                                original=sol or "", error="missing board or solution",
                                resolution="dropped (incomplete data)"))
                print(f"BAD {pack['id']}/{title}: missing board or solution -> dropped")
                dropped += 1
                continue

            # Verify the shipped solution first, so known-bad.md records the real error.
            try:
                par = verify(board, sol)
            except ValueError as shipped_err:
                # Shipped solution is corrupt. Try a re-verified override, else drop.
                if board in overrides:
                    try:
                        par = verify(board, overrides[board])
                    except ValueError as ov_err:
                        bad.append(dict(pack=pack["id"], title=title, index=i, board=board,
                                        original=sol, error=str(shipped_err),
                                        resolution=f"dropped (override also failed: {ov_err})"))
                        print(f"BAD {pack['id']}/{title}: {shipped_err} -> override FAILED -> dropped")
                        dropped += 1
                        continue
                    sol = overrides[board]
                    bad.append(dict(pack=pack["id"], title=title, index=i, board=board,
                                    original=lv["solution"], error=str(shipped_err),
                                    resolution=f"replaced with `{sol}` (par {par})"))
                    print(f"BAD {pack['id']}/{title}: {shipped_err} -> replaced (par {par})")
                    replaced += 1
                else:
                    bad.append(dict(pack=pack["id"], title=title, index=i, board=board,
                                    original=sol, error=str(shipped_err),
                                    resolution="dropped (no replacement found)"))
                    print(f"BAD {pack['id']}/{title}: {shipped_err} -> dropped")
                    dropped += 1
                    continue
    ...
    if bad:
        print("\nbad shipped solutions (see known-bad.md):")
        for r in bad:
            print(f"  {r['pack']}/{r['title']}: {r['resolution']}")
    if dupes:
        print(f"\n{len(dupes)} duplicate board(s) dropped (kept in first pack by order)")
